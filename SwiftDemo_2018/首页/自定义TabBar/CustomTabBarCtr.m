...
@interface CustomTabBarCtr ()<UITableViewDelegate,UITableViewDataSource>
/** dataArrM */
...
/** 强引用 */
@property (nonatomic, strong) CustomTabBarCtr *vc;
@end

@implementation CustomTabBarCtr
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    for (int i = 0; i < titleArr.count; i++) {
        if (i == 0) {
            self.vc = [[CustomTabBarCtr alloc] init];
