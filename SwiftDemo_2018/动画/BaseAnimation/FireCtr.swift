...
class FireCtr: BaseViewController {
    ...
    override func viewDidLoad() {
        ...
        if navigationItem.title == "Fire" {
            fireOne()
        }else if navigationItem.title == "FireTow"{
            fireTow()
        }
    ...
    private func fireTow(){
        view.backgroundColor = UIColor.black
        fireImageView = UIImageView.init(frame: CGRect(x: 100, y: 100, width: 100, height: 100))
        fireImageView.center = view.center
        fireImageView.image = UIImage(named: "zhuguang")
        view.addSubview(fireImageView)
        fireEmitter2.emitterCells = [fire2]
        view.layer.addSublayer(fireEmitter2)
    }
    //创建一个粒子
    lazy var fire2 : CAEmitterCell = {
        let cell = CAEmitterCell()
        ...
        //粒子在xy平面的发射角度
        cell.emissionLongitude = CGFloat(Double.pi + Double.pi * 0.5)
        
        //粒子发射角度容差
        cell.emissionRange = CGFloat(Double.pi * 0.5)
    ...
    //创建一个发射器对象
    lazy var fireEmitter2 : CAEmitterLayer = {
        let emitter = CAEmitterLayer()
        
        //设置中心点
        //        emitter.emitterPosition = CGPoint(x: self.fireImageView.frame.width * 0.6, y: self.fireImageView.frame.height * 0.4)
        
        emitter.emitterPosition = CGPoint(x: self.fireImageView.center.x, y: self.fireImageView.center.y - 30)
